...
Agenda.prototype.saveJob = function (job, cb) {
    ...
    if (props.type == 'single')
        this._db.findAndModify({name: props.name, type: 'single'}, {}, {$set: props}, {upsert: true, new: true}, function(err,doc){
          processDbResult(err,doc && doc.value)
        });
    else {
        if (job.attrs._id) {
            this._db.findAndModify({_id: job.attrs._id}, {}, {$set: props}, {new: true}, function(err,doc){
              processDbResult(err,doc && doc.value)
            });
        }
        else {
